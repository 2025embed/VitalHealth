...
LL_ATON_DECLARE_NAMED_NN_INSTANCE_AND_INTERFACE(tof)
uint8_t *buffer_in;
uint8_t *buffer_out;
...
void MX_X_CUBE_AI_Process(void)
{
  ...
  const LL_Buffer_InfoTypeDef * ibuffersInfos = NN_Interface_tof.input_buffers_info();
  const LL_Buffer_InfoTypeDef * obuffersInfos = NN_Interface_tof.output_buffers_info();
  ...
  for (int inferenceNb = 0; inferenceNb<10; ++inferenceNb) {
    ...
    LL_ATON_RT_Init_Network(&NN_Instance_tof);  // Initialize passed network instance object
    do {
      /* Execute first/next step */
      ll_aton_rt_ret = LL_ATON_RT_RunEpochBlock(&NN_Instance_tof);
      /* Wait for next event */
    ...
    LL_ATON_RT_DeInit_Network(&NN_Instance_tof);
    /* -------------------- */
    /* - End of Inference - */
    /* -------------------- */
  }
